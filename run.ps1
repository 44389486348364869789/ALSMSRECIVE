...
if ($LASTEXITCODE -eq 0) {
    ...
    & "$env:LOCALAPPDATA\Android\Sdk\platform-tools\adb.exe" shell am start -n com.example.alsmsrecive/.MainActivity
    
    Write-Host "Showing logs (Press Ctrl+C to stop)..." -ForegroundColor Yellow
    & "$env:LOCALAPPDATA\Android\Sdk\platform-tools\adb.exe" logcat | Select-String "com.example.alsmsrecive"
} else {
